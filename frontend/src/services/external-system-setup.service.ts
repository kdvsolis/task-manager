...
export class ExternalSystemSetupService {
  private baseUrl = '/api/external-systems'; // Base URL for the API
  ...
  async search(title: string, page: number = 0, size: number = 10) {
    return this.apiService.get(`${this.baseUrl}/search?title=${title}&page=${page}&size=${size}`, {}, {}, '');
  }
}
